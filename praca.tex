...
\begin{equation}
  ...
  \left\{
    \begin{array}{l r l}
      ...
      \}
      \end{split} & \mbox{dla } y=2n, & n\in\mathbb{Z} \\[40pt]
      \begin{split}
        \{
        (x-1, \lfloor\frac{y}{2}\rfloor), \\
        (x, \lfloor\frac{y}{2}\rfloor-1), \\
        ...
        \}
      \end{split} & y=2n+1, & n\in\mathbb{Z} \\
    \end{array} \right.
  \label{nodehexes}
\end{equation}
\begin{equation}
  ...
  \left\{
    \begin{array}{l r l}
      \begin{split}
        ...
        \}
      \end{split} & \mbox{dla } y=3n, & n\in\mathbb{Z} \\[30pt]
      \begin{split}
        \{
        (x-1, \lfloor\frac{y}{3}\rfloor-1), \\
        (x, \lfloor\frac{y}{3}\rfloor-1)
        \}
      \end{split} & y=3n+1, & n\in\mathbb{Z} \\[30pt]
      \begin{split}
      \{
      (x-1, \lfloor\frac{y}{3}\rfloor), \\
      (x, \lfloor\frac{y}{3}\rfloor-1)
      \}
      \end{split} & y=3n+2, & n\in\mathbb{Z} \\
    \end{array} \right.
  \label{edgehexes}
\end{equation}
\begin{equation}
  ...
  \left\{
    \begin{array}{l r@{,~} l}
      \begin{split}
        ...
        (x, y-1)
        \}
      \end{split} & \mbox{dla } y=3n & n\in\mathbb{Z} \\[40pt]
      \begin{split}
        \{
        (x-1, y+2), \\
        (x, y-2), \\
        ...
        (x, y+1)
        \}
      \end{split} & y=3n+1 & n\in\mathbb{Z} \\[40pt]
      \begin{split}
        \{
        (x-1, y+1), \\
        (x, y-1), \\
...
\begin{itemize}
\item \emph{first settlement} - faza budowania pierwszej osady, po
  wybudowaniu osady przechodzimy do fazy \emph{first road}
\item \emph{first road} - faza budowania pierwszej drogi. Jeśli
  istnieje gracz o numerze wyższym niż numer aktualnego gracza, to po
  wybudowaniu drogi przechodzimy do fazy \emph{first settlement} W
  przeciwnym wypadku przechodzimy do \emph{second settlement}
\item \emph{second settlement} - faza budowania drugiej osady, po
  wybudowaniu osady przechodzimy do \emph{second road}
\item \emph{second road} - budowanie drugiej drogi. Jeśli istnieje
  gracz o numerze niższym niż numer aktualnego gracza, to po
  wybudowaniu drogi przechodzimy do fazy \emph{second road}. W
  przeciwnym wypadku następuje przejście do fazy \emph{before roll}
\item \emph{before roll} - faza przed rzutem kością. Wykonując rzut
  kością, przenosimy się do fazy \emph{after roll}. Jeśli wynik rzutu
  kością to 7, to przejście odbywa się poprzez fazy pośrednie - fazę
  \emph{discard} oraz fazę \emph{robber}. Przejście do fazy
  \emph{discard} następuje wówczas, gdy jeden z graczy posiada więcej
  niż 7 kart surowców. Jeśli nie ma takiego gracza, to przechodzimy do
  fazy \emph{robber}. Bezpośrednie przejście do fazy \emph{robber}
  następuje również po zagraniu karty armii
\item \emph{discard} - faza odrzucania nadmiarowych zasobów, w czasie
  której gracz ma obowiązek zredukować ilość posiadanych przez siebie
  kart zasobów do połowy. Faza ta jest powtarzana, dopóki wszyscy
  gracze nie pozbędą się nadmiarowych kart, po czym następuje
  przejście do fazy \emph{robber}
\item \emph{robber} - podczas tej fazy gry, należy przesunąć pionek
  rozbójnika. Po przesunięciu pionka następuje przejście do fazy
  \emph{before roll} lub \emph{after roll}, w zależności od tego, czy
  w aktualnej turze dokonano już rzutu kością
\item \emph{after roll} - główna faza gry, podczas której gracz ma
  ...
  do fazy \emph{road building first road}. Możliwe jest także
  tworzenie ofert handlu z innymi graczami. Po stworzeniu oferty
  następuje przejście do fazy \emph{offer}. Gracz w dowolnym momencie
  może zakończyć fazę \emph{after roll}, co powoduje przejście do fazy
  \emph{before roll}
\item \emph{offer} - faza oferty handlu, podczas której gracz
  składający ofertę oczekuje na odpowiedzi innych. W każdej chwili
  może on zakończyć tę fazę powodując powrót do \emph{after roll}
\item \emph{road building first road} oraz \emph{road building second
    road} - w każdej z faz gracz może wybudować po jednej darmowej
  drodze. Po wybudowaniu przynajmniej jednej z dróg następuje
  przejście do fazy \emph{after roll}
\end{itemize}

Schemat przejść w maszynie stanowej fazy gry został przedstawiony na
\imref{statemachine-phase}. Dodatkowo, gra może znajdować się w jednym
