...
sześciokątów $hexes_{adj}$, wierzchołków $nodes_{adj}$ i krawędzi
$edges_{adj}$ do danego sześciokąta o współrzędnych $(x, y)$ zostały
przedstawione kolejno w równaniach \ref{hexhexes}, \ref{hexnodes} i
\ref{hexedges}. Każdy sześciokąt posiada po sześć sąsiadujących
pozycji elementów każdego typu.
...
sześciokątów $hexes_{adj}$, wierzchołków $nodes_{adj}$ i krawędzi
$edges_{adj}$, z danym wierzchołkiem o współrzędnych $(x, y)$, zostały
przedstawione w równaniach \ref{nodehexes}, \ref{nodenodes} i
\ref{nodeedges}.
...
sześciokątów $hexes_{adj}$, wierzchołków $nodes_{adj}$ i krawędzi
$edges_{adj}$, dla krawędzi o współrzędnych $(x,y)$, zostały
przedstawione w równaniach \ref{edgehexes}, \ref{edgenodes} i
\ref{edgeedges}.
...
\begin{figure}[ht]
  \begin{center}
    \includegraphics[width=0.5\linewidth]{statemachine.pdf}
  \end{center}
  \caption{Maszyna stanowa gry}
  \label{statemachine}
\end{figure}
