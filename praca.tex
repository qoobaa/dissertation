...
\begin{titlepage}
  \begin{center}
    ...
    Przykład zastosowania frameworku Ruby On Rails do stworzenia gry
    internetowej\\[220pt]
  \end{center}
...
\chapter{Wstęp}
Początki sieci \emph{World Wide Web} sięgają lat 90 XX wieku. Na
początkowo całkowicie statycznych stronach internetowych stopniowo
zaczęły pojawiać się dynamiczne elementy. Stało się tak za sprawą
wprowadzenia do przeglądarek internetowych obsługi języka
\emph{JavaScript}. Główną przeszkodą w tworzeniu pełnowymiarowych
aplikacji webowych w architekturze klient-serwer była komunikacja. Aby
odebrać lub wysłać nawet niewielką ilość danych, konieczne było
przeładowanie całego dokumentu \emph{HTML}. Sytuacja ta sprawiła, iż
język \emph{JavaScript} od samego początku postrzegany był bardziej
jako dodatek, niż podstawowe narzędzie mogące posłużyć do budowy
aplikacji. Pierwsze ,,jednoekranowe'' aplikacje webowe, nie wymagające
od użytkownika przeładowywania dokumentu, wykorzystywały dodatkowe
technologie, z których najpopularniejszą do dzisiaj jest \emph{Adobe
  Flash} (wówczas \emph{Macromedia Flash}). To właśnie w firmie
\emph{Macromedia} powstało określenie tego typu aplikacji jako
,,bogatych aplikacji internetowych'' (ang. \emph{Rich Internet
  Applications}). W międzyczasie pojawiły się pierwsze przeglądarki
oferujące możliwość komunikacji z serwerem bez konieczności
przeładowywania strony. Rosnąca popularność technologii \emph{Ajax},
bo o niej mowa, była jednym z powodów okrzyknięcia nowopowstających
aplikacji terminem \emph{Web 2.0}. Niezależnie od tego, czy przełom
określany tym terminem nastąpił czy też nie, oczywistym jest fakt, iż
dzisiejsze aplikacje internetowe znacząco różnią się od swoich
poprzedników.
...
która dziś używana jest głównie do odtwarzania
multimediów\footnote{trwają prace nad wprowadzeniem do piątej
  specyfikacji języka \emph{HTML} znaczników \texttt{<audio>} i
  \texttt{<video>} oraz zintegrowaniu dekoderów z przeglądarkami
  internetowymi} oraz animacji i grafiki wektorowej\footnote{podobne
  możliwości oferuje standard \emph{Scalable Vector Graphics (SVG)} w
  połączeniu z językiem \emph{JavaScript}}. Przekonywujący w tej
sprawie jest również fakt ogłoszenia przez \emph{Microsoft}
rozpoczęcia prac nad \emph{Office Web Apps}. Pomimo promowania przez
firmę \emph{Microsoft} rozwiązania konkurencyjnego dla \emph{Adobe
  Flash} -- technologii \emph{Silverlight}, wszystkie aplikacje
opracowywane w ramach projektu \emph{Microsoft Office Web Apps} w
założeniu mają wymagać od użytkownika wyłącznie przeglądarki
internetowej. Uzasadnieniem takiej decyzji jest fakt, iż nie można
wymagać posiadania dodatkowego oprogramowania od użytkownika
korzystającego z komputera w publicznej bibliotece lub na
lotnisku\cite{microsoft}.

Nie sposób również przecenić potencjału drzemiącego w rynku urządzeń
mobilnych. Z miesiąca na miesiąc powiększa się gama dostępnych
urządzeń, pracujących pod kontrolą rozmaitych platform. Wiele z tych
platform nie oferuje wsparcia dla technologii takich jak \emph{Java}
czy \emph{Adobe Flash}. Tworzenie aplikacji webowych to często
najtańszy i najszybszy sposób dotarcia do szerokiej grupy odbiorców i
uniezależniania tworzonego oprogramowania od platform
sprzętowych. Zauważalnej zmianie ulega również model rozwoju
oprogramowania, które coraz częściej oferowane jest klientowi w formie
usługi. Najważniejszymi korzyściami płynącymi z zastosowania takiego
podejścia jest przerzucenie obowiązków zarządzania, pielęgnowania i
aktualizowania oprogramowania na jego dostawcę.

\section{Cel pracy}
Wiele popularnych dziś serwisów oferujących gry sieciowe, takie jak
szachy czy brydż, wymaga do działania \emph{Javy} lub
\emph{Flasha}. Celem niniejszej pracy dyplomowej jest ukazanie, że
możliwe jest zaimplementowanie nawet dość skomplikowanej gry
sieciowej, mając do dyspozycji wyłącznie funkcje współczesnych
przeglądarek internetowych.

Zaprojektowany i zaimplementowany zostanie serwis umożliwiający
rozgrywanie wieloosobowych partii gry planszowej ,,Osadnicy z
Catanu''. Aplikacja ma umożliwiać prowadzenie wielu rozgrywek w tym
samym czasie, dzięki tworzeniu tzw. stołów, podobnie jak w serwisie
\url{kurnik.pl}. Część serwerowa aplikacji będzie wykorzystywać
framework \emph{Ruby on Rails}. Komunikacja serwera z klientem ma się
odbywać zgodnie z zasadami \emph{REST}\footnote{REST -
  ang. Representational State Transfer}. Zastosowanie \emph{REST} nie
wprowadza żadnych ograniczeń dla technologii użytej w części
klienckiej, jednakże jej podstawowa implementacja wykorzystywać ma
wyłącznie możliwości przeglądarek internetowych, w szczególności język
\emph{JavaScript}. Dzięki takiemu podejściu, dużo prostsze będzie
przystosowanie gry do działania np. w urządzeniach mobilnych.

\cleardoublepage

\chapter{Analiza zagadnienia i istniejących rozwiązań}
...
wersji gry. Moim celem nie jest tłumaczenie zasad gry, a jedynie
ukazanie najważniejszych jej aspektów, które musiały zostać
uwzględnione przy implementacji. Na końcu rozdziału przedstawionych
zostało kilka powszechnie dostępnych, działających rozwiązań. Niektóre
z nich zawierają pewne modyfikacje w stosunku do podstawowych zasad
gry.
...
\section{Element losowy - kości do gry}
...
Karta rycerza stanowi element walki, aspekt ten został dokładniej
omówiony w rozdziale \ref{sec:fight}. Karty postępu w różny sposób
przyspieszają rozwój gracza. Jedną z ciekawszych kart rozwoju
jest,,punkt zwycięstwa'', karta stanowiąca istotny element zaskoczenia
w grze. W związku z tym, że po ,,zakupie'' karty rozwoju jest ona
zakryta dla pozostałych graczy, nie znają oni jej rodzaju dopóki nie
zostanie zagrana. Karty ,,punkt zwycięstwa'' ujawniane są bezpośrednio
przed końcem gry. W rezultacie gracz posiadający np. trzy karty
rozwoju, może potencjalnie posiadać trzy dodatkowe punkty zwycięstwa,
które umożliwią mu znacznie szybsze zakończenie gry.
...
Wszystkie zaprezentowane implementacje gry wymagają od użytkownika
posiadania dodatkowego oprogramowania (poza przeglądarką
internetową). Większość z nich jest przystosowana do jednej konkretnej
platformy (np. PC z systemem Microsoft Windows). Wadą tego typu
rozwiązań jest to, że przystosowanie ich dla innych platform (np. dla
urządzeń mobilnych) często wiąże się z koniecznością przepisania całej
aplikacji.

\subsection{Pioneers}\label{sec:pioneers}
Pierwotna nazwa tej aplikacji brzmiała \emph{gnocatan}. Aplikacja
została udostępniona w 2000 roku na zasadach licencji GNU GPL. Gra
została napisana w języku \emph{C}, podzielona jest na serwer i część
kliencką. Na stronie projektu możemy znaleźć kod źródłowy i wersje
skompilowane na wszystkie popularne platformy
...
Aplikacja została napisana w Javie, rozwijana jest od początku 2004
roku. Podobnie jak w przypadku gry \emph{Pioneers}, została ona
udostępniona na zasadach licencji GNU GPL\cite{jsettlers}. Aplikacja
przystosowana do uruchamiania w przeglądarce internetowej z
zainstalowanym środowiskiem uruchomieniowym Javy (ang. \emph{Java
  Runtime Environment}). Na \imref{jsettlers} został przedstawiony
zrzut ekranu z gry \emph{JSettlers}.
...
\emph{WiFi}. Nie posiadają one niestety żadnego centralnego serwera
gier. Obie opisywane wersje są płatnym oprogramowaniem własnościowym.
...
rozwój aplikacji, cały jej kod dostępny jest na zasadach Powszechnej
Licencji Publicznej GNU Affero w wersji 3\cite{affero}.

Do zaimplementowania części serwerowej aplikacji zostanie wykorzystany
język \emph{Ruby} i framework \emph{Ruby on Rails}. Wybór ten można
uzasadnić bardzo dobrym wsparciem frameworka dla architektury
\emph{REST}, zgodnie z którą będą komunikować się obie części
aplikacji.
...
tworzenia aplikacji użytkowych, serwerów sieciowych, jak i skryptów
...
\item \emph{ActiveResource} - biblioteka umożliwiająca mapowanie
  zasobów \emph{REST} na modele w aplikacji
\end{itemize}
...
po wydaniu \emph{Merba 1.0}, ogłoszone zostało połączenie obu grup
programistów\cite{merge}. Utworzony w ten sposób zespół zajmuje się
połączeniem powstającego \emph{Merba 2} i \emph{Ruby on Rails
  3}. Zgodnie z ustaleniami, pierwsze wydania \emph{Ruby on Rails 3}
mają się pojawić pod koniec 2009 roku.
...
\subsection{HTTP Push (Comet)}\label{sec:push}
...
rozproszonych. Skrót ten pochodzi od ang. \emph{Representational State
  Transfer}, został użyty w pracy doktorskiej Roya Fieldinga
...
jest prostota tego rozwiązania. Stopniowo wypiera ono protokoły takie
jak \emph{XML-RPC} czy \emph{SOAP}, czego najlepszym dowodem są
\emph{Amazon Web Services}. Klienci tych usług mają swobodny wybór
pomiędzy dostępem do nich poprzez interfejs \emph{REST} i \emph{SOAP},
jednak około 80\% z nich decyduje się na użycie
...
aplikację, jest przechowywanie stanu gry w relacyjnej bazie
danych. Konsekwencją tego faktu jest konieczność podziału aplikacji na
modele odzwierciedlające relacje bazy danych. Przy projektowaniu
niektórych modeli, w szczególności planszy i jej składników, wzorowano
się na działającej implementacji -- grze \emph{Pioneers} opisywanej w
rozdziale \ref{sec:pioneers}.
...
współrzędnych budowanej przez gracza drogi. Okazuje się zatem, że
...
kolumn. Podobnie jak w przypadku wierzchołka -- zajęta krawędź musi
należeć do jednego z graczy.
...
\begin{itemize}
\item \texttt{preparing} - początkowy stan, pozwalający na
  dołączanie się do gry nowych graczy. Przejście do stanu
  \texttt{playing} następuje po dołączeniu się przynajmniej trzech
  graczy, jeśli wszyscy z nich są gotowi do gry
\item \texttt{playing} - gra znajdująca się w tym stanie została już
  rozpoczęta. Przejście do stanu \texttt{ended} następuje po uzyskaniu
  przez jednego z graczy minimum 10 punktów zwycięstwa
\item \texttt{ended} - końcowy stan oznaczający grę zakończoną
\end{itemize}
...
\ref{javascript}, do komunikacji wybrany został \emph{HTTP
  Pull}. Opóźnienia, wynikające z zastosowania tego rozwiązania nie
powinny mieć większego wpływu na rozgrywkę. Istotnym elementem
przemawiającym za wykorzystaniem \emph{HTTP Pull} jest prostota
użytkowania. Nie wymusza ono posiadania dodatkowych serwerów i
domen. Ponadto, większość dostępnych bibliotek \emph{JavaScriptowych}
udostępnia wygodne sposoby tworzenia wywołań \emph{Ajax}. Rozwiązanie
to upraszcza tworzenie i testowanie aplikacji, jego wydajność może
jednak okazać się niewystarczająca aby obsłużyć dużą liczbę
jednoczesnych połączeń. W środowisku ,,produkcyjnym'' konieczne może
być wykorzystanie techniki \emph{HTTP Push}, opisanej w rozdziale
\ref{sec:push}.
...
statystyk po zakończeniu rozgrywki (np. statystyki wyników rzutów
kośćmi).
...
wykorzystana została biblioteka \emph{State Machine}, autorstwa
...
\begin{listing}
  \input{statemachine}
  \listingcaption{Maszyna stanowa stanu gry}
...
stanów w postaci ciągu znaków\footnote{nic nie stoi na przeszkodzie,
  ...
  \emph{Enumerate By}, autorstwa Aarona Pfeifera}. Oprócz stanów,
...
\begin{listing}
  \input{board}
  \listingcaption{Tworzenie przykładowej planszy do gry}
  \label{listing:board}
\end{listing}
...
\begin{listing}
  \input{hex}
  \listingcaption{Wyciąganie sąsiadujących sześciokątów}
  \label{listing:hex}
\end{listing}

\begin{listing}
  \input{find}
  \listingcaption{Przykładowe zapytanie \emph{SQL}}
  \label{listing:find}
\end{listing}
...
\begin{listing}
  \input{node}
  \listingcaption{Maszyna stanowa wierzchołka}
  \label{listing:node}
\end{listing}
...
\begin{listing}
  \input{edge}
  \listingcaption{Implementacja algorytmu liczenia najdłuższej drogi}
  \label{listing:edge}
\end{listing}
...
\begin{listing}
  \input{player}
  \listingcaption{Maszyna stanowa gracza}
...
\begin{listing}
  \input{card}
  \listingcaption{Maszyna stanowa karty rozwoju}
  \label{listing:card}
\end{listing}
...
Zgodnie z zasadami gry, karta rozwoju ,,punkt zwycięstwa'' ma
natychmiastowy efekt (nie jest zagrywana). Dla uproszczenia
...
\item przechowywanie ,,posolonego''\footnote{,,solenie hasła'' polega
  ...
  \emph{SHA-1}\footnote{ang. Secure Hash Algorithm}
...
\begin{listing}
  \input{usersession}
  \listingcaption{Implementacja i obsługa uwierzytelniania użytkowników}
...
\begin{listing}
  \input{filter}
  \listingcaption{Implementacja klasy \texttt{EdgesController}}
...
\texttt{false}. Dzieje się tak np. wówczas, gdy niezalogowany
...
\begin{listing}
  \input{routes}
  \listingcaption{Zdefiniowanie ścieżek}
...
\begin{listing}
  \input{boardtable}
  \listingcaption{Struktura znaczników \emph{HTML} tabeli -- planszy do gry}
  \label{listing:boardtable}
\end{listing}

\begin{listing}
  \input{hexes}
  \listingcaption{Fragment kodu źródłowego w języku \emph{Sass},
...
poprzednich wersji, biblioteka \emph{YUI 3} została przeprojektowana i
...
modelu \emph{DOM} zaczerpnięte z \emph{jQuery}. \emph{YUI 3} oferuje
...
Należy zaznaczyć fakt, że biblioteka \emph{YUI 3} znajduje się ciągle w
...
środowiskach produkcyjnych. Konsekwencją wyboru \emph{YUI 3} jest
...
\emph{YUI 3} oraz jej rozszerzenia. Pomimo tych niedogodności,
...
\begin{listing}
  \input{ruby}
  \listingcaption{Kompilacja i instalacja interpretera języka \emph{Ruby}}
  \label{listing:ruby}
\end{listing}
...
\begin{listing}
  \input{git}
  \listingcaption{Pobranie kodu z repozytorium \emph{Git}}
  \label{listing:git}
\end{listing}
...
\begin{listing}
  \input{application}
  \listingcaption{Instalacja wymaganych bibliotek, konfiguracja i
...
\begin{listing}
  \input{script_server}
  \listingcaption{Uruchomienie serwera aplikacji}
...
Osiągnięte zostały najważniejsze cele projektowe. Użytkownik, po
zalogowaniu się do serwisu, ma możliwość utworzenia nowej gry i
dołączenia do niej. Do tworzenia losowych planszy zaimplementowany
został również prosty generator, którego algorytm działania zbliżony
jest do oryginalnych zasad gry. Zalogowani użytkownicy mają oczywiście
możliwość dołączenia do gier, które nie zostały jeszcze rozpoczęte. Po
zgromadzeniu odpowiedniej liczby graczy możliwe jest przeprowadzenie
rozgrywki.

Zgodnie z oczekiwaniami, najtrudniejszym do wykonania elementem
projektu było zaimplementowanie części klienckiej aplikacji. Fakt ten
spowodowany jest przez kilka różnych czynników. Język
\emph{JavaScript} jest dość specyficznym językiem programowania, który
w pewnych kwestiach (w szczególności w kwestii dziedziczenia
prototypowego) znacząco różni się od popularnych rozwiązań i często
wymaga od programisty zmiany sposobu myślenia. Wykorzystana w
projekcie biblioteka \emph{YUI 3} znajduje się ciągle w fazie
dynamicznego rozwoju, jest ona również stosunkowo mało
popularna. Ponadto, testowanie kodu aplikacji napisanej w języku
\emph{JavaScript} nie jest zadaniem prostym. Wszystkie te
niedogodności wpłynęły niekorzystnie na jakość kodu tej części
aplikacji. Z całą pewnością kod ten nie jest pozbawiony błędów, lecz
jego konstrukcja powoduje, że błędy te nie uniemożliwiają
przeprowadzenia rozgrywki. W każdym momencie (np. po wystąpieniu
błędu) możliwe jest przeładowanie dokumentu, co skutkuje ponownym
uruchomieniem aplikacji.

Konsekwencją zastosowania frameworka \emph{Ruby on Rails} jest
przechowywanie stanu gry w bazie danych. Rozwiązanie takie jest z całą
pewnością mniej wydajne niż w przypadku utworzenia dedykowanego
serwera gier, tak jak ma to miejsce w grze \emph{Pioneers}, opisywanej
w rozdziale \ref{sec:pioneers}. Ma ono jednak niepodważalną zaletę --
w znaczącym stopniu uodparnia aplikację na błędy i awarie. Dzięki
niemu konieczność zrestartowania serwera aplikacji nie powoduje utraty
danych o toczących się rozgrywkach. Może się to okazać niezwykle
przydatne, np. w przypadku zaoferowania użytkownikom możliwości
uczestnictwa w płatnych turniejach.

\section{Dalszy rozwój aplikacji}
Przed utworzeniem ogólnodostępnego serwisu z grą ,,Osadnicy z
Catanu'', niezbędne jest wykonanie następujących prac:

\begin{itemize}
\item przystosowanie aplikacji do stabilnej wersji biblioteki
  \emph{YUI 3.0.0}
\item zaprojektowanie przyjaznego dla użytkownika interfejsu
  graficznego
\item dodanie czatu, umożliwiającego komunikację między graczami
\end{itemize}

Niezbędne może się także okazać wykonanie pewnych optymalizacji, z
których najbardziej oczywistą wydaje się być przyspieszenie
generowania stanu gry w formacie \emph{JSON}. Framework \emph{Ruby on
  Rails} oferuje wiele gotowych rozwiązań, mogących posłużyć do
zwiększenia wydajności tego fragmentu aplikacji, np. \emph{Rails
  Metal}\footnote{dzięki temu rozwiązaniu możliwe jest obsłużenie
  pewnej grupy zapytań jeszcze przed ich przetworzeniem i przekazaniem
  do aplikacji \emph{Ruby on Rails}} lub
\emph{Etags}\footnote{technika ta pozwala serwerowi \emph{HTTP} na
  stwierdzenie, czy żądany przez przeglądarkę zasób został
  zmodyfikowany od czasu ostatniego zapytania. Eliminuje to
  konieczność ponownego wysyłania tych samych danych}. Po dostosowaniu
aplikacji do tradycyjnych przeglądarek internetowych, możliwe jest
także opracowanie wersji dedykowanych dla przeglądarek mobilnych. W
przypadku wersji mobilnej konieczne jest dostosowanie interfejsu do
niewielkiego ekranu oraz do różnic w obsłudze zdarzeń myszy.

Możliwości dalszego rozwoju aplikacji są niemalże
nieograniczone. Wynika to głównie z faktu istnienia bardzo dużej
liczby dodatków i modyfikacji zasad gry. Użytkownikom przyszłego
serwisu można także zaoferować wprowadzenie rozbudowanych statystyk i
rankingów oraz możliwość organizowania turniejów. Zdecydowanie
wykracza to poza możliwości jednego programisty. Jest to istotny
powód, dla którego kod aplikacji udostępniony został na zasadach
Powszechnej Licencji Publicznej \emph{GNU}.
