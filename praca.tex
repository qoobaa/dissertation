...
metodzie \texttt{longest\_road}, klasy \texttt{Edge}. Pierwszym jej
parametrem jest \texttt{visited\_roads} - lista odwiedzonych już
wierzchołków, których nie należy brać pod uwagę przy
obliczeniach. Rezultatem jej wywołania jest para elementów: najdłuższa
znaleziona droga oraz uzupełniona lista odwiedzonych
wierzchołków. Każda droga może posiadać maksymalnie cztery
sąsiadujące drogi - dwie lewe i dwie prawe. W pierwszej kolejności
liczone są długości dróg znajdujących się po lewej stronie, z
pominięciem krawędzi już odwiedzonych.

\begin{figure}[ht]
  \begin{center}
    \includegraphics[width=0.5\linewidth]{longest-road.pdf}
  \end{center}
  \caption{Przykładowa droga o długości 8}
  \label{road}
\end{figure}

%%

%% skip_roads służy do pominięcia drugiej drogi jedynie w pierwszym
%% wywołaniu metody. Gwarantuje to, że droga ta nie zostanie pominięta
%% w przypadku wystąpienia cyklu.
